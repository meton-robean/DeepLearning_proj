...
char_to_ix = { ch:i for i,ch in enumerate(chars) } #form unique char dict pair:  char:i
ix_to_char = { i:ch for i,ch in enumerate(chars) } #form unique char dict pair:  i:char 
...
def gen_epoch_data(raw_data, batch_size):
    ...
    for i in range(num_batches):
        ...
        for j in range(batch_size):
            inp = raw_data[idx + j*seq_length:idx + (j+1)*seq_length]  # one seq of raw_data 
            target = raw_data[idx + 1+(j*seq_length):idx + 1+((j+1)*seq_length)]  # target y labels :the next seq of inp

            batch.append([c2i(inp), c2i(target)]) # equal to 'X-Y data'
        epoch_data.append(batch) #add one 'X-Y data pair each time'
    return epoch_data
...
rnn_inputs = tf.unstack(x_oh) #col-vector??
